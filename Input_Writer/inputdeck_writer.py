...
class inputdeck:
    ...
    modify_nodes     : modifies self.temporalnodedata variable to allow printing of time series
    modify_timedata  : adds timestep data, ie. time values of various timesteps
    """
    ...
    def __init__(self, file, shadowoffset=1000, lastECSWElement=99999999):
        ...
        self.shellsetticketpos = []
        self.shellcolticketpos = []
        self.shellgeneralticketpos = []
                ...
                if ticket_started and not lc_line.startswith("*"):
                    ...
                    if lc_line.startswith("*"):
                        ...
                        if lc_line.startswith("*node"):
                            ...
                            if "generate" in lc_line  or "add" in lc_line:
                                pass
                            elif "general" in lc_line:
                                self.shellgeneralticketpos.append(ticket_number)
                            elif "column" in lc_line:
    ...
    def reduce_nodes(self, rm_element_list, node_range, output_tickets):
        '''
        ...
        rm_node_ids
        rm_node_ids_ext
        omitted_nodes            list of external node IDs which have been removed.
        '''
        ...
        omitted_elements = list(set(self.elementdata[:,0]) - set(rm_elements[:,0]))
                    ...
                    if node_keyword_started:
                        if (linecontent.startswith("*node")):
                            ...
                            if int_nid not in rm_node_ids:
                                ...
                                omitted_nodes.append(self.nodeids[int_nid])
                            int_nid +=1
                    else:
                        # waiting for next *NODE keyword
                        if linecontent.startswith("*node"):
        ...
        return rm_node_ids, rm_node_ids_ext, new_node_tickets, omitted_nodes, omitted_elements

    #def clean_node_sets(self, tickets, red_nodes_intIDs=[]):
    #pass

    #def clean_element_sets(self, tickets, red_elem_intIDs=[]):
    #pass
    ...
    def modify_nodes(self, node_ids, node_data):
        ...
        if node_data.ndim == 1 or node_data.shape[1] == 1:
            ...
            temporalnodedata = np.empty((self.nodedata.shape[0], 3, node_data.shape[1]))
    ...
    # Function used to modify time data
    ...
    def write_reduced_inputdeck(self, red_elem_set, contact_elem_set=[], lastECSWElement=1e11, newfilename=None, \
        ...
        if not keep_all_nodes:
            rm_nodes, rm_nodes_ext, new_node_tickets, omitted_nodes, omitted_elements = self.reduce_nodes(rm_elements, node_reduction_range, output_tickets)
            n_ticket_id = 0
            for i in self.nodeticketpos:
                output_tickets[i] = new_node_tickets[n_ticket_id]
                n_ticket_id += 1
            output_tickets = self.remove_nodes_from_sets(output_tickets, omitted_nodes, omitted_elements)
    ...
    def remove_nodes_from_sets(self, output_tickets, omitted_nodes, omitted_elements):        
        ...
        # Remove nodes from $SET_SHELL_LIST & *SET_SHELL
        list_shell_tickets = sorted(self.shellsetticketpos) 
        new_list_shell_tickets = [deepcopy(output_tickets[i]) for i in list_shell_tickets]
        for i, list_ticket in enumerate(new_list_shell_tickets):
            list_shells = []
            new_list_ticket = []
            new_list_shell = ""
            skipped = 1
            if "title" in list_ticket[0].lower():
                ...
                skip = 1
            for list_shell in list_ticket:
                if list_shell.startswith("$"):
                    if skip > 0:
                        skipped += 1
                    continue
                elif list_shell.startswith("*"):
                    if skip > 0:
                        skipped += 1
                    continue
                elif skip > 0:
                    ...
                    for num in string_slices(list_shell, [10, 10, 10, 10, 10, 10, 10, 10])[0]:
                        try:
                            list_shells.append(int(num))
                        except:
                            pass
            list_shells = set(list_shells).difference(omitted_elements)
            j = 1
            for x in sorted(list_shells):
                new_list_shell += str(x).rjust(10)
                if j%8 == 0:
                    new_list_ticket.append(new_list_shell + "\n")
                    new_list_shell = ""
                    j = 0
                j += 1
            if new_list_shell != "":
                new_list_ticket.append(new_list_shell + "\n")
            new_list_shell_tickets[i] = list_ticket[:skipped] + new_list_ticket

        for i, j in enumerate(list_shell_tickets):
            output_tickets[j] = new_list_shell_tickets[i]
    ...
    def get_elements(self):
        return self.elementdata, self.ecsw_elementtypes, self.exteid2ints        
...
if __name__ == "__main__":
    ...
    if run:
        ...
        id_mask = np.where(ids < 58000000)[0]

        coordinates_mask = np.empty((id_mask.shape[0]*3))
        coordinates_mask[0::3] = id_mask*3
        coordinates_mask[1::3] = id_mask*3 + 1
        coordinates_mask[2::3] = id_mask*3 + 2
        coordinates_mask = coordinates_mask.astype(int)
        ids = ids[id_mask]

        coordinates = coordinates[coordinates_mask,:]

        deck.modify_nodes(ids, coordinates)
        nodedata, nodeids, _ = deck.get_nodes()
        print(nodedata.shape)

        with open("basis_vectors.pkl", "rb") as f:
            V = pickle.load(f)
        
        V = V[coordinates_mask,:]            
