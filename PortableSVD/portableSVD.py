...
# from Projects.CarCrashModel.Data.data import nodes as xsection_data
...
if __name__ == '__main__':
	...
	output_name								=		available_input_files[int(choose_full_input_file)]
	full_input_file_path					=  		os.path.join(data_folder_path, output_name)

	### Determining the input file with simplified simulation data
	supported_simplified_input_types 		= 		[".npz"]
	...
	output_name								=		available_nodal_input_files[int(choose_nodal_input_file)]
	...
	if vtkMapping:
		available_vtk_files					=		[os.path.basename(x) for x in glob.glob('%s*_0*%s'%(visualization_folder_in_path, ".vtk"))]
