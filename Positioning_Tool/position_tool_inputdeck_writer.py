...
eltype_dict = {"*element_solid" : "solid", "*element_beam" : "beam", "*element_shell" : "shell"}
...
def element_data_to_ticket(element_data, rm_elems=None):
    ...
    for element in element_data:
        ...
        i = 2
        for element_node in element[2:]:
            if element_node == -1:
                break
            ticket += str(element_node).rjust(8)
        ticket += "\n"
...
class inputdeck:
    ...
    def __init__(self, file, shadowoffset=1000, lastECSWElement=99999999):
        ...
        self.read_elements()
        self.read_includes()
        self.temporalnodedata = self.nodedata

    def read_includes(self):
        if "/" in self.filename:
            self.directory = "/".join(self.filename.rsplit("/")[:-1])
        else:
            self.directory = ""
        include_tickets = [self.tickets[i] for i in self.includeticketpos]
        for ticket in include_tickets:
            for line in ticket[1:]:
                if not line.startswith("$"):
                    include_name = join(self.directory,line.strip())
                    self.includenames.append(include_name)
                    # print(include_name)
                    self.includes.append(inputdeck(include_name))
    ...
    def read_elements(self):
        ...
        # First loop to count the elements
            for i, linecontent in enumerate(ticket):
                if element_keyword_started:
                    if linecontent.strip() in eltype_dict.keys():
        ...
        for ticket in element_tickets:
            ...
            for i, linecontent in enumerate(ticket):
                if element_keyword_started:
                    element_keyword = linecontent[:linecontent.find("\n")]
                    # If new element keyword exists, update eltype to match
                    if linecontent.startswith("*element_") and eltype_dict[element_keyword] != eltype:
                        eltype = eltype_dict[element_keyword]
                    elif linecontent.startswith("*"):
                        # new keyword which is not a element keyword!
                        element_keyword_started = False
                    elif linecontent.startswith("$"):
                        ...
                        if eltype is "shell":        # internal code: shell = 0
                            ...
                            self.elementdata[int_eid,0:11] = np.array([ext_eid, 3, slices[1], slices[2], slices[3], slices[4], slices[5], slices[6], slices[7], slices[8], slices[9]])    
    ...
    def write_inputdeck(self, newfilename=None, append="mod"):
        for include in self.includes:
            # print(include.filename)
            include.write_inputdeck(append=append)

        output_tickets = deepcopy(self.tickets)
        if newfilename is None:
            newfilename = self.filename.rsplit(".",1)[0] + "_" + append + ".key"
        else:

            newfilename = join(self.directory,newfilename)

        node_data = self.get_modified_nodes()
        element_data = self.get_elements()    

        nodes_appended = False
        elements_appended = False
        with open(newfilename, "w+") as f:
            for i, ticket in enumerate(output_tickets):
                keyword = ticket[0]
                ...
                if nodes_appended == False:
                    if "*node" in lc_line:
                        node_ticket = node_data_to_ticket(node_data)
                        f.write(node_ticket)
                        nodes_appended = True
                        continue
                if elements_appended == False:
                    if keyword in eltype_dict.keys():
                        element_ticket = element_data_to_ticket(element_data)
                        f.write(element_ticket)
                        elements_appended = True
                        continue
                if "*node" in lc_line or keyword in eltype_dict.keys():
                    continue
                if "*include" in lc_line:
                    for i,line in enumerate(ticket[1:]):
                        if not line.startswith("$"):
                            # print(line)
                            ticket[i+1] = line.rsplit(".")[0] + "_" + append + ".key\n"
                for ic, c in enumerate(ticket):
                    if c.startswith("*database_nodout"):
                        nextline = ticket[ic+1]
                        j = 0
    ...
    def modify_nodes(self, node_ids, node_data):
        for include in self.includes:
            include.modify_nodes(node_ids, node_data)
        try:
            timesteps = node_data.shape[1]
        except:
            timesteps = 1
        mask_array = np.isin(node_ids, self.nodeids)
        node_ids = node_ids[mask_array]
        node_data = node_data.reshape((-1,3))[mask_array, :]

        updated_nodes = [self.extnid2int[i] for i in node_ids]

        if timesteps == 1:
            self.temporalnodedata[updated_nodes,:] = node_data
        else:
            temporalnodedata = np.empty((self.node_count, 3, node_data.shape[1]))
            for timestep in range(timesteps):
                temporalnodedata[:,:,timestep] = deepcopy(self.nodedata)
                temporalnodedata[updated_nodes,:,timestep] = node_data[:,timestep]
    ...
    def get_nodes(self):
        node_data = deepcopy(self.nodedata)
        node_ids = deepcopy(self.nodeids)
        extnid2int = deepcopy(self.extnid2int)
        for include in self.includes:
            include_node_data, include_node_ids, include_extnid2int = include.get_nodes()
            if node_data.size != 0:
                if include_node_data.size != 0:
                    node_data = np.concatenate((node_data, include_node_data))
                    node_ids = np.concatenate((node_ids, include_node_ids))
            else:
                node_data = include_node_data
                node_ids = include_node_ids
            extnid2int.update(include_extnid2int)
        
        return node_data, node_ids, extnid2int

    def get_modified_nodes(self):
        return self.temporalnodedata, self.nodeids, self.extnid2int
...
if __name__ == "__main__":
    ...
    if run:
        ...
        deck = inputdeck("open_source_sim/01_inputs/VIVA_OpenHBM_F50_Geom_20161202.key")
