...
def writetoOutput(output_name, A_r, node_ids):
	node_start = "*NODE"
	node_end   = "*END"
	node_ids   = iter(node_ids)
	A_r        = iter(A_r[:,-2])
	with open(output_name, 'r', encoding='utf-8') as file_in:
		...
		for line in node_line_data:
			if(re.match(" [0-9]",line)):
				data = str(next(node_ids)).rjust(8) + str(next(A_r)).rjust(16) + str(next(A_r)).rjust(16) + str(next(A_r)).rjust(16) 
	...
	print(output_name[:-2] + "_3.k")
	with open(output_name[:-2] + "_3.k", 'w', encoding='utf-8') as file_out:
